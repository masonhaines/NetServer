...
void UTcpClient::PollSocket()
{
	...
	while(ClientSocket->HasPendingData(PendingDataSize)) 
	{
        ...
        while(true)
        {
            ...
            // }
        	if (PartialJsonBytes.Num() < 5) break;
        	...
        	TArray<uint8> Frame;
        	Frame.Append(PartialJsonBytes.GetData(), Framelength);

        	// consume frame + "<END>"
        	PartialJsonBytes.RemoveAt(0, Framelength + 5);
        	...
        	FUTF8ToTCHAR Utf8ToTchar(Utf8Ptr, Framelength);
        	const FString JsonStringFromBytes(Utf8ToTchar.Length(), Utf8ToTchar.Get());

            // PartialJsonBytes.RemoveAt(0, Framelength + DelimiterLength);
