...
void UTcpClient::PollSocket()
{
	...
	while(ClientSocket->HasPendingData(PendingDataSize)) 
	{
        ...
        while(true)
        {
        	...
        	if (ReassemblyBuffer.Num() < 5) break; // outer check, not enough bytes to contain delimiter or additional data

            const int32 limit = ReassemblyBuffer.Num() - DelimiterLength; // limit is the size of the buffer minus the length of delimiter
            ...
            for (int32 i = 0; i <= limit; ++i)
            {
                ...
                // verify for  "<END>"
                if (ReassemblyBuffer[i] == (uint8)'<' && ReassemblyBuffer[i+1] == (uint8)'E' && ReassemblyBuffer[i+2] == (uint8)'N'
                    && ReassemblyBuffer[i+3] == (uint8)'D' && ReassemblyBuffer[i+4] == (uint8)'>')
                {
                    IndexOfDelimiter = i;
                    break;
                }
